...
func seedRoot(ctx context.Context, db *postgres.DB, emitter postgres.Emitter, log *xlog.Logger) error {
	...
	// Mint a project-admin token so the panel can manage the seeded project
	// immediately (logged once; not persisted in plaintext).
	token, err := op.MintAdminToken(ctx, proj.ID)
	if err != nil {
		return err
	}
	log.Info("seeded root project",
		xlog.String("project_id", proj.ID),
		xlog.String("admin_token", token),
	)
	return nil
}
